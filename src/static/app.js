...
function renderRecommendations() {
    ...
    hackathonsList.forEach(h => {
        ...
        card.className = "flex flex-col gap-4 p-5 bg-[#0a0a0a] border border-[#444] rounded-sm hover:border-[#555] transition-colors group";
        
        card.innerHTML = `
            <div class="flex justify-between items-start">
                <div class="flex flex-col gap-1">
                    <span class="font-mono text-[10px] uppercase tracking-wider text-[#999]">${escapeHTML(h.organization)}</span>
            ...
            ${h.match_reason ? `
            <div class="mt-2 p-3 bg-transparent border-l border-[#444] font-mono text-[11px] text-[#bbb] leading-relaxed italic">
                <span class="text-[#ccc] font-semibold mr-1">ANALYSIS:</span> ${escapeHTML(h.match_reason)}
            </div>
            ` : ""}
            
            <div class="flex items-center justify-between mt-auto pt-4 border-t border-[#444]">
                <a href="${h.url}" target="_blank" class="font-mono text-[11px] text-[#999] hover:text-[#fbbf24] flex items-center gap-2 transition-colors">
                    LINK <i class="fa-solid fa-arrow-right text-[8px]"></i>
                </a>
                <button class="btn-track font-mono text-[10px] px-3 py-1 border transition-all ${isTracked ? 'bg-[#ff4757] border-[#ff4757] text-white' : 'bg-transparent border-[#444] text-[#999] hover:border-[#666] hover:text-[#e8e8e8]'}" data-url="${h.url}">
